...
interface ClosedLeadsViewProps {
    role: "csr" | "admin";
    // Agent whose leads to show; defaults to the logged-in user
    csrId?: string;
    backHref: string;
    backLabel: string;
    title?: string;
...
// off the working dashboard once closed and are read-only here. Shared by
// the CSR's own page and the admin's view of a single agent.
export default function ClosedLeadsView({ role, csrId, backHref, backLabel, title = "Closed Leads" }: ClosedLeadsViewProps) {
    ...
    const fetchLeads = useCallback(async () => {
        setLoading(true);
        try {
            const userId = csrId || await getUserId();
    ...
    // Share of all closed leads, rounded to one decimal (0 when empty)
        ...
        <ProtectedRoute role={role}>
            ...
            <div className="min-h-screen bg-[#F4F7FE] p-4 lg:p-10">
                <div className="max-w-[1600px] mx-auto mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                    <div>
                        <Link href={backHref} className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-slate-800 mb-2">
                ...
                <div className="max-w-[1600px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                    ...
                    <SummaryCard title="Paid" value={stats.paid.toString()} percentage={percentOfTotal(stats.paid)} icon={<FiCheckCircle />} color="green" />
                    <SummaryCard title="Revenue" value={stats.revenue.toLocaleString()} icon={<FiDollarSign />} color="green" />
                    <SummaryCard title="Not Interested" value={stats.notInterested.toString()} percentage={percentOfTotal(stats.notInterested)} icon={<FiSlash />} color="orange" />
                    <SummaryCard title="Wrong Number" value={stats.wrongNumber.toString()} percentage={percentOfTotal(stats.wrongNumber)} icon={<FiPhoneOff />} color="rose" />
                </div>
