...
export default function AdminDashboardPage() {
    ...
    return (
        <div className="p-4 md:p-8 space-y-8 bg-[#F8FAFC] min-h-screen text-slate-900 font-sans relative">
            ...
            >
                <div className="text-center md:text-left">
                    ...
                    <p className="text-slate-400 text-sm font-medium italic uppercase tracking-wider">
                        {filter}ly Performance Monitoring
                    </p>
                </div>
                ...
                <div className="flex flex-wrap justify-center gap-3">
                    <button
                        ...
                        className="relative z-10 bg-slate-900 text-white px-6 py-3 rounded-2xl font-bold text-sm hover:scale-105 active:scale-95 transition-all shadow-lg"
                    >
                        + Create CSR
                    </button>
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-6">
                <div className="flex bg-white p-2 rounded-2xl shadow-sm border border-slate-200/60">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 w-full lg:w-auto flex-1">
                    <SummaryCard title="Filtered Leads" value={filteredLeadsByTime.length} trend="Live" />
                    <SummaryCard title="Total Sales" value={data?.totalSales || 0} color="green" />
                    <SummaryCard title="Revenue" value={`PKR ${data?.totalRevenue || 0}`} color="blue" />
                    <SummaryCard title="Conv Rate" value={data?.conversionRate || "0%"} color="purple" />
                </div>
            </div>
                ...
                <section className="col-span-12 lg:col-span-9 space-y-8">
                    ...
                    >
                        <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                            ...
                            <span className="text-xs font-bold bg-indigo-100 text-indigo-600 px-3 py-1 rounded-full uppercase tracking-tighter">
                                Showing {filter}
                            </span>
                        </div>
                        <CSRLeadsPanel
                    ...
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200/60 shadow-sm">
                        <h3 className="text-xl font-black text-slate-800 tracking-tight mb-8">Performance Analytics</h3>
                    ...
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex justify-center items-center z-[100] p-4">
                        ...
                        >
                            ...
                            <div className="flex gap-4">
                                ...
                                <button onClick={confirmExcelUpload} disabled={uploading} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-lg active:scale-95 transition-all">{uploading ? "Importing..." : "Confirm & Import"}</button>
                            </div>
                        </motion.div>
                    </div>
                )}

                {/* Create CSR Modal (CORRECTED) */}
                {showCSRModal && (
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex justify-center items-center z-[100] p-4">
                        <motion.div
                            initial={{ scale: 0.9, opacity: 0 }}
